...
def process_recording() -> Response:
    ...
    logging.info("FormData=%s", payload)

    response = VoiceResponse()
    started_at = time.perf_counter()
    recording_url = request.values.get("RecordingUrl", "").strip()
    call_sid = request.values.get("CallSid", "anonymous-session").strip() or "anonymous-session"
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("Starting waitress server on port %s", port)
        serve(app, host="0.0.0.0", port=port, threads=8)
    except Exception as error:
        logging.warning("Waitress unavailable, falling back to Flask dev server: %s", str(error))
